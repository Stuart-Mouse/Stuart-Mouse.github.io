...
void render_particle(Particle* p, Vec2 offset) {
    ...
    if (viewport.w >= viewport.h) {
        render_scale = p->scale * (float)viewport.w / (float)p->texture_clip.w;
    } else {
        render_scale = p->scale * (float)viewport.h / (float)p->texture_clip.w;
    }
...
void update_particle_emitter(Particle_Emitter* e) {
    ...
    if (ticks_now > e->next_emit_time) {
        ...
        if (p != NULL) {
            ...
            p->fadein_time  = e->fadein_time;
            p->fadeout_time = e->fadeout_time;

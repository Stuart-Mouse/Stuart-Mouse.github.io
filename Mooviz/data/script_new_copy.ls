...
planet.position = Vector2.{ 0.5, 0.5 } + ellipse(cycle, 2, 1, 3.0/8.0) * Vector2.{ radius/2, radius };
planet.rotation = cos(cycle * tau) * 2.0;
planet.size.x = 0.05;
...
for 1..10 {
    ...
    trail := Vector2.{ 0.5, 0.5 } + ellipse(lerp, 2, 1, 3.0/8.0) * Vector2.{ radius/2, radius };
    draw_square(trail, 0.01, cos(lerp * tau) * 2.0, planet.color);
}

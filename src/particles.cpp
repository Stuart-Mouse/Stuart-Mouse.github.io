...
bool update_particle(Particle* p) {
    ...
    p->color_mod.a = max_alpha;
    
    if (lifetime_lerp < fade_time) {
        p->color_mod.a *= lifetime_lerp / fade_time;
    } else if (lifetime_lerp > 1.0 - fade_time) {
        p->color_mod.a *= ((1.0 - lifetime_lerp) / fade_time); // this is probably dumb
    }
    
    // use lifetime lerp to do things like mod color and alpha over time
...
void render_particle(Particle* p, Vec2 offset) {
    ...
    Vec2 render_size     = (to_vec2(p->texture_clip.size) * p->scale) * window_render_scale; 
    // Vec2 render_size     = Vec2 { p->scale * window_render_scale, p->scale * window_render_scale }; 
    Vec2 render_position = (p->position * window_render_scale) - (render_size / 2.0) + offset;
    
    SDL_Rect rect = {
        .x = (int)render_position.x,
        .y = (int)render_position.y,
        .w = (int)render_size.x,
        .h = (int)render_size.y
    };
    ...
    if (p->texture.id != NULL) {
        SDL_SetTextureColorMod(p->texture.id, 
            (uint8_t)clamp(p->color_mod.r * 255.0, 0, 255),
            (uint8_t)clamp(p->color_mod.g * 255.0, 0, 255),
            (uint8_t)clamp(p->color_mod.b * 255.0, 0, 255)
        );
        SDL_SetTextureAlphaMod(p->texture.id, 
            (uint8_t)clamp(p->color_mod.a * 255.0, 0, 255)
        );
        SDL_RenderCopyEx(
            renderer, p->texture.id, 
            &p->texture_clip.sdl, &rect,
            p->rotation, NULL, 
            SDL_FLIP_NONE
        );
    }

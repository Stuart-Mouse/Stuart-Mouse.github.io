...
Module Sound_Player platform layer inserted from C:/jai/modules/Toolchains/Web/libjs/Sound_Player.js
...
jai_imports.js_file_open = (name_count, name_data, name_is_constant, for_writing, keep_existing_content, log_errors, out_file, out_success) => {
    const value = wasm_pause(); // 0 => paused, + => ok, - => not ok
    if (value === 0) (async () => {
        const name = copy_string_to_js(name_count, name_data, name_is_constant);
        ...
        const file_handle = await opfs_find_file(name, create_file);
        ...
        file_handle.seek_position = 0;
        file_handles[file_index]  = file_handle;
        return file_index+1;
    })().then(wasm_resume); else if (value === -1) {
        log_resume_error();
        new DataView(jai_exports.memory.buffer).setInt32(Number(out_success), 0, true);
    } else {
        const file_index = value-1;
        const view = new DataView(jai_exports.memory.buffer);
...
jai_imports.js_file_close = (file_index_pointer) => {
    const view = new DataView(jai_exports.memory.buffer);
...
jai_imports.js_file_delete = (filename_count, filename_data, filename_is_constant) => {
    ...
    case 0: (async () => {
        ...
        const file_handle = await opfs_find_file(name);
        if (file_handle === undefined) {
            set_resume_error(`Could not delete "${name}": File not found.`);
            return -1;
        }
        await file_handle.remove();
        return +1;
    })().then(wasm_resume); break;
    case +1: return true;
    case -1: {
...
// Think again! These writablestreams only modify files on disk when they are closed womp womp
jai_imports.js_file_write = (file_index_pointer, data, count) => {
    const view        = new DataView(jai_exports.memory.buffer);
    const file_index  = view.getInt32(Number(file_index_pointer), true);
    const file_handle = file_handles[file_index];
    if (file_handle == undefined) {
        jai_log_error(`Could not write to file with invalid handle ${file_index}`);
        return false;
    }
    ...
    if (value === 0) (async () => {
        ...
        try {
            const writer = await file_handle.createWritable({ keepExistingData: true });
            await writer.seek(file_handle.seek_position);
            await writer.write(buffer);
            await writer.close();
            file_handle.seek_position += buffer.byteLength;
            wasm_resume(+1);
        } catch (e) {
            set_resume_error(`Could not write to ${file_handle.name} (${e.name}) ${e.message}`);
            wasm_resume(-1);
        }
    })(); else if (value < 0) {
        log_resume_error();
...
jai_imports.js_file_read = (file_index, data, bytes_to_read, total_read_pointer) => {
    const file_handle = file_handles[file_index];
    if (file_handle == undefined) {
        jai_log_error(`Could not write to file with invalid handle ${file_index}`);
        return false;
    ...
    if (value === 0) (async () => {
        try {
            ...
            const buffer  = await new Promise((resolve, reject) => {
                const slice    = file.slice(file_handle.seek_position, file_handle.seek_position+to_read+1);
                const reader   = new FileReader();
                reader.onload  = () => { resolve(reader.result); };
                reader.onerror = () => { reject(reader.error);   };
                reader.readAsArrayBuffer(slice);
            });
            file_handle.seek_position += to_read;
            ...
            set_resume_error(`Could not read from ${file_handle.name} (${e.name}) ${e.message}`);
            wasm_resume(-1);
        }
    })(); else if (value < 0) {
        log_resume_error();
...
jai_imports.js_file_length = (file_index, length_pointer) => {
    const file_handle = file_handles[file_index];
    if (file_handle == undefined) {
        jai_log_error(`Could not write to file with invalid handle ${file_index}`);
        return false;
    ...
    if (value === 0) (async () => {
        try {
            ...
            set_resume_error(`Could not get size of ${file_handle.name} (${e.name}) ${e.message}`);
            wasm_resume(-1);
        }
    })(); else if (value < 0) {
        log_resume_error();
...
jai_imports.js_file_set_position = (file_index, position) => {
    const file_handle = file_handles[file_index];
    if (file_handle == undefined) {
        jai_log_error(`Could not write to file with invalid handle ${file_index}`);
        return false;
    }
    file_handle.seek_position = Number(position);
    return true;
};

const read_entire_file_buffers = [];
jai_imports.js_read_entire_file = (file_index, zero_terminated, out_content, out_success) => {
    const file_handle = file_handles[file_index]
    if (file_handle === undefined) {
        jai_log_error(`Could not read file with invalid file handle ${file_index}.`);
        return;
    }
    
    const value = wasm_pause(); // 0 => paused, other => read_entire_file_buffers_index+1
    if (value === 0) (async () => {
        const file   = await file_handle.getFile();
        ...
        read_entire_file_buffers[index] = buffer;
        wasm_resume(index+1);
    })(); else {
        const index  = value-1;

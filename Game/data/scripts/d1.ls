...

{
    ...
    foreach (plat1, plat2) {
        ...
        it.offset_next = it.root_offset + circle(cycle_lerp, 0) * platform_range;
        it.flags = it.flags | .POSITIONED_BY_SCRIPT;
    }
    ...
    for 0..fireball_count-1 {
        ...
        fireball_position := ellipse2(cycle_lerp, p1, p2, 1, 1, 1, 2, 0);
...

{
    ...
    plat3.offset_next = center + circle(cycle_lerp,     0) * platform_range;
    plat4.offset_next = center + circle(cycle_lerp+0.5, 0) * platform_range;
    
    plat3.flags = plat3.flags | .POSITIONED_BY_SCRIPT;
    plat4.flags = plat4.flags | .POSITIONED_BY_SCRIPT;
    ...
    for 0..fireball_count-1 {
        ...
        fireball_position := ellipse2(cycle_lerp, p1, p2, 0.7, 2, 1, 3, 0);
        immediate_fireball(fireball_id, fireball_position, .{1,1});
    }
}
